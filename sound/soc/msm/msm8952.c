...
parse_mclk_freq:
	...
	spk_pa_gpio = of_get_named_gpio(pdev->dev.of_node,
					"ext-spk-amp-gpio", 0);
	if (spk_pa_gpio < 0) {
		dev_err(&pdev->dev, "%s: error! spk_pa_gpio is %d\n",
	...
	if (headset_gpio < 0) {
		dev_err(&pdev->dev, "%s: error! headset_gpio is %d\n",
			__func__, headset_gpio);
	} else {
		if (gpio_request_one(headset_gpio, GPIOF_DIR_OUT,
			"headset_enable"))
