...
static ssize_t led_flash_show(struct device *dev,
...
static ssize_t led_flash_store(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf,
			       size_t size)
{
...
static int32_t msm_gpio_flash_low(struct msm_flash_ctrl_t *flash_ctrl,
...
static int32_t msm_gpio_flash_high(struct msm_flash_ctrl_t *flash_ctrl,
				   struct msm_flash_cfg_data_t *flash_data)
{
	CDBG("Enter\n");
...
static int32_t msm_gpio_flash_off(struct msm_flash_ctrl_t *flash_ctrl,
				  struct msm_flash_cfg_data_t *flash_data)
{
	CDBG("Enter\n");
...
void msm_gpio_torch_brightness_set(struct led_classdev *led_cdev,
...
static struct led_classdev msm_gpio_torch_led = {
	.name		= "flashlight",
	.brightness_set	= msm_gpio_torch_brightness_set,
	.brightness	= LED_OFF,
};
	...
	void __user *argp)
{
	...
	case CFG_FLASH_INIT:
		msm_gpio_torch_led.brightness = 0;
		msm_torch_led[0].brightness = 0;
...
static int32_t msm_flash_platform_probe(struct platform_device *pdev)
{
	...
	} else if (flash_ctrl->flash_driver_type == FLASH_DRIVER_GPIO) {
		rc = led_classdev_register(&pdev->dev, &msm_gpio_torch_led);
		if (rc)
			pr_err("Failed to register gpio led dev\n");
	}

	rc = device_create_file(&pdev->dev, &dev_attr_led_flash);
	if (rc < 0)
		pr_err("Creation of led_flash_node file failed!\n");
