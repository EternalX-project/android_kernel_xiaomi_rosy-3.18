...
static void set_color_delayed(struct work_struct *ws)
{
	...
	pr_debug("%s: color = %d, brightness = %d, blink = %d \n", __func__,
		color,
		aw2013->leds[color].brightness,
			...
			const struct i2c_device_id *id)
{
	...
	if (0 != aw2013_i2c_check_device(client)) {
		dev_err(&client->dev,
				"%s:  aw2013_i2c_check_device failed.",
					__func__);
		goto exit2;
	}

	blink_frequency_adjust = of_property_read_bool(client->dev.of_node,
